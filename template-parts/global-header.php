<header id="global-header" class="has-ivory-buff-background-color">
	<nav id="header" class="navbar navbar-expand-md container" aria-label="Global Navigation">
			<div class="navbar-toggler" data-bs-toggle="collapse" aria-controls="navbar" aria-expanded="false" aria-label="<?php esc_attr_e(
     "Toggle global navigation",
     "rbn-pw"
   ); ?>">
				<div><input type="checkbox" class="visually-hidden" id="hi">
				  	<label id="toggle-label" class="menu me-auto" for="hi"><span><?php echo _e(
...
wp_nav_menu([
     "menu_class" =>
       "navbar-nav p-0 m-0 ms-md-auto has-sans-serif-font-family strtoupper order-2",
     "container" => "",
				...
				<ul id="language-switcher" data-no-translation class="d-flex py-5 py-lg-2">
					<?php foreach ($array as $name => $item) { ?>
							<li class="p-0">
								...
								<a href="<?php echo $item["current_page_url"]; ?>" tabindex="1"><?php echo trim(
  htmlentities($item["language_name"])
); ?></a>
								<?php else: ?>
    ...
    if (is_home() || is_front_page()): ?>
      ...
      in_array($post->post_type, ["news", "events", "contributor"])
    ): ?>
				<h1 class="fw-lighter order-2"><?php echo get_post_type_object($post->post_type)
      ->label; ?></h1>
    ...
    ?>
     ...
     "template-parts/header-spotlight",
     "header-spotlight"
   ); ?>
			</div>
		</div><!-- end .row -->
