...
if (isset($post->ID)) {
  ...
  if (!in_array($base, $archives)) {
    if (
      isset($spotlight["show_spotlight"]) &&
      $spotlight["show_spotlight"] == 1
    ) { ?>
	  <?php if (
     isset($spotlight["spotlight_copy"]) &&
     !empty($spotlight["spotlight_copy"])
   ): ?>
    <div class="spotlight">
      <p class="fst-italic"><?php echo $spotlight["spotlight_copy"]; ?></p>
      <?php if (
        isset($spotlight["spotlight_attribution"]) &&
        !empty($spotlight["spotlight_attribution"])
      ): ?><cite class="text-end">&mdash; <?php echo $spotlight[
  "spotlight_attribution"
]; ?></cite><?php endif; ?>
    </div>
	<?php endif; ?>
  <?php }
    ...
    if ($base == "archive-events") {
      ...
      if (is_active_sidebar("events-header-spotlight-attribution")) {
        dynamic_sidebar("events-header-spotlight-attribution");
      }
    } elseif ($base == "archive-news") {
      if (is_active_sidebar("news-header-spotlight")) {
        dynamic_sidebar("news-header-spotlight");
      }
      if (is_active_sidebar("news-header-spotlight-attribution")) {
        dynamic_sidebar("news-header-spotlight-attribution");
      }
    }
  }
}
